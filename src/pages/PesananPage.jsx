              ...
              <tbody className="divide-y">
                {filteredOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 font-mono font-bold text-blue-600">{order.order_code}</td>
                    ...
                    <td className="px-4 py-3 font-bold">{formatRupiah(order.total_price)}</td>
                    <td className="px-4 py-3">
                      {order.discount > 0 ? (
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${STATUS_STYLE[order.status]}`}>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h3 className="text-xl font-bold">{selected.order_code}</h3>
                <p className="text-sm text-gray-400">{selected.created_at}</p>
              </div>
              <span className={`px-3 py-1 rounded-full text-sm font-semibold ${STATUS_STYLE[selected.status]}`}>
            ...
            {selected.pelanggan && (
              <div className="bg-blue-50 rounded-xl p-3 mb-4">
                ...
                <p className="font-semibold">{selected.pelanggan.nama}</p>
            ...
            <div className="space-y-2 mb-4">
              ...
              {selected.items?.map((item, i) => (
                <div key={i} className="flex justify-between text-sm bg-gray-50 rounded-lg p-3">
                  <div>
                    <p className="font-semibold">{item.produk_name}</p>
                    <p className="text-gray-400 text-xs">{item.quantity} x {formatRupiah(item.price)}</p>
                  </div>
              ...
              {selected.discount > 0 && (
                <>
                  <div className="flex justify-between text-sm text-gray-500">
                    <span>Subtotal</span>
                    <span>{formatRupiah(Number(selected.total_price) + Number(selected.discount_amount))}</span>
                  </div>
